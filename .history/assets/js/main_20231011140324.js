window.addEventListener("load", function () {
        ...
        this.listMusicRender = {
            ...
            render: function () {
                ...
                const htmls = `<div class="song" data-index="${self.currentSongIndex}">
                        <h2 class="name_song">${song.name}</h2>
                        <p>${song.singer}</p>
    ...
    MusicMp3.prototype.randomMusic = function () {
        const random = this.listMusicRender.songs[(Math.floor(this.listMusicRender.songs.length *  Math.random()))];
        console.log(random)
        this.audio.src = random.path;
        const nameSong = document.querySelector(".name_song")
        console.log(nameSong.textContent)
        nameSong.textContent = this.listMusicRender.songs[(Math.floor(this.listMusicRender.songs.length *  Math.random()))].name
        this.audio.play();
        this.listMusicRender.render()
        this.playing = true;
        this.btnPlay.classList.remove("fa-play");
