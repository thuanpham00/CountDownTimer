window.addEventListener("load", function () {
    ...
    function checkTimer() {
        ...
        const timer = setInterval(console.log(song.currentTime), 100);
        currentTimeText.textContent = formatTimeMusic(song.currentTime);
        if (!duration) {
            durationTimeText.textContent = "0:00";
        } else {
            durationTimeText.textContent = formatTimeMusic(song.duration);
        }
    }
    ...
    const timer = this.setInterval(checkTimer)
