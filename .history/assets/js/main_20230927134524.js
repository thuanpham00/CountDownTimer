...
//     function setTimer(yearInput, monthInput, dateInput, dayInput, hourInput) {
...
//         const yearInput = this.elements["year"].value;
//         const dayInput = this.elements["day"].value;
//         const hourInput = this.elements["hour"].value;
//         setTimer(dateInput, monthInput, yearInput, dayInput, hourInput);
//     });
// });
...
window.addEventListener("load", function () {
    ...
    let timerInterval; // Biến để lưu trữ ID của setInterval
    ...
    function setTimer(yearInput, monthInput, dateInput, dayInput, hourInput) {
        ...
        // Dừng bất kỳ đếm ngược nào đang chạy trước đó
        clearInterval(timerInterval);
        // Bắt đầu đếm ngược
        timerInterval = setInterval(changeTimer, 500);
        changeTimer(); // Cập nhật ngay khi gọi hàm này
    }
    ...
    formTimer.addEventListener("submit", function (e) {
        ...
        const dayInput = this.elements["day"].value;
        const hourInput = this.elements["hour"].value;
        setTimer(yearInput, monthInput, dateInput, dayInput, hourInput);
    });
});
