window.addEventListener("load", function () {
    ...
    function MusicMp3() {
        ...
        const listRenderMusic = setTimeout(() => this.printListMusic(), 200);

        this.itemMusic = document.querySelectorAll(".img-2");
        const handleClickMusic = setTimeout(() => this.handleClickItemMusic(),200)
    }
    new MusicMp3();
    ...
    MusicMp3.prototype.printListMusic = function () {
        const that2 = this;
        that2.listMusicRender.songs.forEach((item, index) => {
            const itemMusic = `<div class="song song-row itemMusic" data-index="${index}">
    ...
    `;
            that2.listMusic.insertAdjacentHTML("beforeend", itemMusic);
        });
    };


    MusicMp3.prototype.handleClickItemMusic = function() {
        this.itemMusic.forEach(item => console.log(item))
    }
});
